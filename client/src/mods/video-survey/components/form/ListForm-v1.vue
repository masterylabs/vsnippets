<template>
	<div>
		<drag-sort v-model="items" height="37" :disabled="inEdit" @input="onSorted" class="mb-4">
...
export default {
	...
	beforeMount() {
		if (this.value.length) this.items = [...this.value]
	},
	...
	methods: {
		...
		doDuplicate(item) {
			...
			// place below current
			const index = this.items.findIndex((a) => a.id == item.id) + 1
