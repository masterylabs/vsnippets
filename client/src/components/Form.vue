<template>
	<form @submit.prevent="submit">
		<v-card class="py-4 px-3">
				...
				<template v-for="(field, index) in getFields">
					<v-expand-transition :key="field.value">
						<div v-show="!field.hide || !field.hide(val)">
							<slot v-if="slotFields && slotFields.indexOf(field.value) > -1" :name="field.value" />
			...
			<v-btn v-if="canClose" large absolute right top icon @click="$emit('close')"
...
export default {
	...
	computed: {
		...
		getTitle() {
			return this.title ? this.title : this.form && this.form.title ? this.form.title : ''
		},
		getFields() {
	...
	methods: {
		...
		async submit() {
			this.loading = true
			const data = await this.$app.post(this.getEndpoint(), this.val)
