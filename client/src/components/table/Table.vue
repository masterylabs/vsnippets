<template>
	<div>
		<v-card rounded="lg">
			<v-toolbar flat>
				<v-badge :content="total | bigNumber" @click="selectAll">
					...
					<v-btn slot="badge" text rounded dark class="mt-n2 mx-n3" small @click="selectAll">
						{{ total | bigNumber }}
					</v-btn>
				</v-badge>
				...
				<slot name="add-new-btn">
					<v-btn class="ml-4" outlined color="primary" @click="create = true">Add New</v-btn>
				</slot>
			</v-toolbar>
			...
			<v-expand-transition>
				<v-row v-if="selected.length || isQuickSelectAll" no-gutters>
					<v-chip class="ml-4 mt-2" tile color="success" close loading @click:close="deselectAll">
					...
					<!-- <v-chip
						:close="isQuickSelectAll"
						small
						dark
						color="grey darken-4"
						@click="quickSelectAll"
						@click:close="quickSelectAll"
					>
						quickSelectAll
					</v-chip> -->
				</v-row>
			...
			<v-card-text>
				...
				>
					...
					<template v-if="isQuickSelectAll" v-slot:[`item.data-table-select`]="{}">
						<v-checkbox
							v-model="isQuickSelectAll"
							checked
							readonlyx
					...
					<template v-for="name in itemSlots" :slot="`item.${name}`" slot-scope="{ item }">
					...
					<!-- Name linkCols -->
					<template v-for="primaryCol in linkCols" v-slot:[`item.${primaryCol}`]="{ item }">
						<v-btn
					...
					<template v-slot:[`item.actions`]="{ item }">
						<template v-if="deleteId == item.id">
							<v-btn small color="error" :loading="deleting" @click="deleteItem">Delete</v-btn>
							<v-btn small depressed class="ml-1" @click="deleteId = null">Cancel</v-btn>
						</template>
						<template v-else>
							<v-tooltip v-for="action in actions" :key="action.value" top>
								<template v-slot:activator="{ on, attrs }">
									<v-btn v-bind="attrs" v-on="on" icon @click="onAction(action.value, item)">
							...
							<div v-if="activeSelected.length" class="pt-4 pb-2">
								<slot name="group-actions" :selected="activeSelected">
									<v-row align="center" justify="start" class="my-4" no-gutters>
										<v-row v-if="isSelectedDelete" no-gutters align="center" justify="start">
											<v-btn color="error" :loading="deletingSelected" @click="deleteSelected"
												>Delete Selected</v-btn
											>
											<v-btn class="ml-1" depressed @click="isSelectedDelete = false"
												>Cancel</v-btn
											>
										</v-row>
										<template v-else>
											<v-tooltip v-for="action in groupActions" :key="`se${action.value}`" top>
												<template v-slot:activator="{ on, attrs }">
													<v-btn v-bind="attrs" v-on="on" icon @click="action.selectedClick">
														<v-icon>{{ action.icon }}</v-icon>
													</v-btn>
												</template>
		...
		<!-- <v-row>
			<dev-raw v-if="items" :value="items[0]" />
			<dev-raw v-if="editItem" :value="editItem" />
		</v-row> -->
	</div>
</template>
